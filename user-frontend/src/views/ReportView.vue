...
async function refreshStatus() {
  ...
  try {
    status.value = await getReportStatus(props.taskId)
  } catch {
    status.value = null
  }
}
...
async function startReport() {
  ...
  pollCount.value = 0
  try {
    const res = await generateReport(props.taskId)
    if (res?.generation_id) {
      generationId.value = res.generation_id
      pollResult(res.generation_id)
    }
  } catch (e) {
...
async function pollResult(targetGenerationId) {
  const maxPolls = 90
  for (let i = 0; i < maxPolls; i++) {
    await new Promise(r => setTimeout(r, 2000))
    try {
      const html = await getReportResult(targetGenerationId)
      pollCount.value = i + 1
...
function downloadReport() {
  window.open(getReportDownloadUrl(generationId.value), '_blank')
}

function downloadMarkdown() {
  window.open(getMarkdownUrl(generationId.value), '_blank')
}

watch(() => props.taskId, refreshStatus, { immediate: true })
</script>
...
<template>
  <div class="report-view">
    <header class="view-header stagger-1">
      ...
      <p class="view-desc">基于当前研究任务的结构化裁决组装报告，并由 LLM 进行轻度润色</p>
    </header>
    ...
    <section class="generate-section stagger-3">
      <div class="generate-card">
        <div class="generate-input-wrap">
          <input
            :value="researchQuery || '请先启动一次研究'"
            class="generate-input"
            readonly
          />
          <button class="generate-btn" @click="startReport" :disabled="generating || !taskId || !status?.prepared">
        ...
        <p class="generate-hint">报告结构和结论由 Host 裁决确定，LLM 仅优化摘要、衔接与语言表达</p>
      </div>
    </section>
